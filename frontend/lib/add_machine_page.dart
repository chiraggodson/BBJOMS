...
class _AddMachinePageState extends State<AddMachinePage> {
  ...
  Future<void> _save() async {
    ...
    final body = {
      ...
      'kg_per_hour': double.tryParse(_kgPerHourController.text.trim()) ?? 0,
    };
    ...
    try {
      ...
      // Some APIs return {success:false,...} with HTTP 200.
  ...
  String _errorFromResponse(
    ...
    if (rawBody.trim().isNotEmpty) {
      return 'Failed to add machine (HTTP $statusCode): ${rawBody.trim()}';
    }

    return 'Failed to add machine (HTTP $statusCode).';
  }
