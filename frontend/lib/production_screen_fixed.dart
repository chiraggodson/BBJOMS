...
class _ProductionPageState extends State<ProductionPage> {
  ...
  List<_LiveMachineRow> get _rows {
    ...
    for (final machine in _machines) {
      ...
      final status = row.machine.status.trim().toLowerCase();
      final matchesStatus = switch (_statusFilter) {
        'running' => status == 'running',
        'idle' => status == 'idle' || status == 'stopped' || status == 'paused',
        'maintenance' => status == 'maintenance',
        _ => true,
      };

      if (matchesStatus &&
          (query.isEmpty || row.searchText.contains(query))) {
        rows.add(row);
      }
    }
              ...
              child: Column(
                children: [
                  ...
                  _ProductionFilters(
                    ...
                    allCount: _machines.length,
                    runningCount: _runningMachines,
                    idleCount: _machines.where((machine) {
                      final status = machine.status.trim().toLowerCase();
                      return status == 'idle' ||
                          status == 'stopped' ||
                          status == 'paused';
                    }).length,
                    maintenanceCount: _statusCount('maintenance'),
                    onChanged: (value) {
    ...
    return Column(
      children: rows.map((row) {
        return Padding(
          ...
          child: _MachineProductionCard(row: row),
        );
      }).toList(),
    );
  }
...
class _MachineProductionCard extends StatelessWidget {
  final _LiveMachineRow row;

  const _MachineProductionCard({
    required this.row,
  });

  @override
  Widget build(BuildContext context) {
    final machineNo = row.machine.machineNo.isEmpty
        ? '—'
        : row.machine.machineNo;
    final status = row.machine.status.isEmpty
        ? 'Unknown'
        : row.machine.status;
    final hasJob = row.job != null;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: row.machine.status.trim().toLowerCase() == 'running'
              ? BBTheme.border
              : BBTheme.borderSoft,
        ),
      ),
      child: LayoutBuilder(
        builder: (context, constraints) {
          if (constraints.maxWidth < 850) {
            return _MobileMachineProductionCard(row: row);
          }
            ...
            children: [
              Row(
                ...
                children: [
                    ...
                    child: hasJob
                        ? Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 3),
                              Text(
                                '${row.job!.partyName} • ${row.job!.fabricName}',
                                overflow: TextOverflow.ellipsis,
                                style: const TextStyle(
                                  color: BBTheme.muted,
                                  fontSize: 10,
                  ...
                  Expanded(
                    flex: 3,
                    child: Text(
                      hasJob && row.job!.fabricName.isNotEmpty
                          ? row.job!.fabricName
                          : '—',
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(
              ...
              Row(
                children: [
                  _ProductionMetric(
                    label: 'TODAY',
                    value: '${_formatNumber(row.todayKg)} kg',
                    width: 105,
                  ),
                  const SizedBox(width: 26),
                  _ProductionMetric(
                    label: 'PRODUCED',
                    value: '${_formatNumber(row.totalKg)} kg',
                    width: 115,
                  ),
                  const SizedBox(width: 26),
                  _ProductionMetric(
                    label: 'TARGET',
                    value: '${_formatNumber(row.targetKg)} kg',
                    width: 105,
                  ),
                  const SizedBox(width: 28),
                  Expanded(
                    child: _ProductionProgress(
                      totalKg: row.totalKg,
...
class _MobileMachineProductionCard extends StatelessWidget {
  final _LiveMachineRow row;

  const _MobileMachineProductionCard({
    required this.row,
  });
...
class _ProductionProgress extends StatelessWidget {
  final double totalKg;
  final double targetKg;
  ...
  @override
  Widget build(BuildContext context) {
    final percent = targetKg > 0 ? progress * 100 : 0.0;
      ...
      children: [
        Row(
          children: [
            ...
            const Spacer(),
            Text(
              targetKg > 0 ? '${percent.toStringAsFixed(1)}%' : '—',
              style: const TextStyle(
                color: BBTheme.muted,
                fontSize: 9,
                fontWeight: FontWeight.w800,
...
class _ProductionFilters extends StatelessWidget {
          ...
          return Padding(
            ...
            child: InkWell(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: active
                      ? BBTheme.red.withValues(alpha: .12)
                      : BBTheme.black3,
                  borderRadius: BorderRadius.circular(18),
                  border: Border.all(
                    color: active
                        ? BBTheme.red.withValues(alpha: .45)
                        : BBTheme.borderSoft,
                  ),
                ),
                child: Row(
